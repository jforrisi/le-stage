...
class VentasDevolucionesCabezal(models.Model):
    """Cabezal de devolución de venta/nota de crédito"""
    ...
    tipo_documento = models.ForeignKey(
        Documento,
        on_delete=models.PROTECT,
        related_name='devoluciones_ventas',
        verbose_name='Tipo de Documento',
        limit_choices_to={'codigo__in': ['devmovcli', 'tncredit', 'ncreexpo']},
    )
    ...
    disponibilidad = models.ForeignKey(
        Disponibilidad,
        on_delete=models.PROTECT,
        related_name='devoluciones_ventas_contado',
        blank=True,
        null=True,
    ...
    def save(self, *args, **kwargs):
        ...
        # Asignar tipo_documento por defecto como 'devmovcli' si no está definido
    ...
    def actualizar_totales(self):
        ...
        VentasDevolucionesCabezal.objects.filter(transaccion=self.transaccion).update(
...
class VentasDevolucionesLineas(models.Model):
    """Líneas de devolución de venta/nota de crédito"""
    ...
    id_articulo = models.ForeignKey(
        Articulo,
        on_delete=models.PROTECT,
        related_name='devoluciones_ventas_lineas',
        verbose_name='Artículo',
        db_column='id_articulo',
    )
    
    # Referencia a la línea de venta afectada (opcional)
    id_venta_linea = models.ForeignKey(
        'ventas_ingreso.VentasLineas',
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
        related_name='devoluciones',
        verbose_name='Línea de Venta',
        help_text='Línea específica de la factura de venta afectada (opcional)',
    )
    
    # Campos de display (se calculan desde id_venta_linea, pero se guardan para referencia)
    serie_doc_afectado = models.CharField(
        max_length=20,
        blank=True,
        null=True,
        verbose_name='Serie Doc Afectado',
        help_text='Serie del documento de venta afectado (calculado desde id_venta_linea)',
    )
    ...
    numero_doc_afectado = models.CharField(
        ...
        help_text='Número del documento de venta afectado (calculado desde id_venta_linea)',
    )
    ...
    def save(self, *args, **kwargs):
        # Si hay id_venta_linea, calcular serie y numero desde la factura
        if self.id_venta_linea:
            venta = self.id_venta_linea.transaccion
            self.serie_doc_afectado = venta.serie_documento or ''
            self.numero_doc_afectado = venta.numero_documento or ''
            
            # Si hay id_venta_linea, usar su precio_neto como precio inicial
            if not self.precio or self.precio == 0:
                self.precio = self.id_venta_linea.precio_neto
        else:
            # Si no hay id_venta_linea, limpiar campos de documento afectado
        ...
        # Verificar si es devolución movimiento cliente (devmovcli) - no cobra IVA
        es_movcli = self.transaccion.tipo_documento and self.transaccion.tipo_documento.codigo == 'devmovcli'
        
        # Obtener IVA del artículo (pero si es movcli, el IVA será 0)
        if es_movcli:
            # Movimiento Cliente: no cobra IVA, siempre es 0
            iva_valor = Decimal('0')
        ...
        if self.transaccion.precio_iva_inc == 'SI':
            ...
            if iva_valor > 0:
                ...
                # Sin IVA (devmovcli o artículo sin IVA)
