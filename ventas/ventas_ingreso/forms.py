...
class VentasCabezalForm(forms.ModelForm):
    """Formulario para el cabezal de venta"""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        # Configurar tipo_documento - permitir movcli, efactura y factexpo
        if 'tipo_documento' in self.fields:
            self.fields['tipo_documento'].queryset = Documento.objects.filter(
                codigo__in=['movcli', 'efactura', 'factexpo']
            )
            # Si es nueva venta, establecer 'efactura' por defecto (factura, no movimiento cliente)
            if not self.instance.pk:
                try:
                    doc_default = Documento.objects.get(codigo='efactura')
        ...
        # Establecer fecha_documento por defecto a la fecha actual si es nueva venta
        ...
        if self.instance and self.instance.pk and self.instance.id_cliente:
            ...
            # Cargar forma de pago del cliente como opción por defecto
            if cliente.forma_pago and cliente.forma_pago != 'NO_ASIGNADA':
                try:
                    forma_pago_obj = PlazoPago.objects.get(codigo=cliente.forma_pago)
                    # Agregar opciones de plazo
                    opciones_plazo = [
                        (cliente.forma_pago, forma_pago_obj.descripcion),
                        ('elegir', 'Elegir'),
    ...
    def clean(self):
        ...
        # En ventas, precio_iva_inc siempre es 'SI'
        ...
        tipo_venta = cleaned_data.get('tipo_venta', 'CONVENCIONAL')
        ...
        # Validaciones específicas para venta simplificada
        if tipo_venta == 'SIMPLIFICADA':
            if not sub_total or sub_total < 0:
                raise forms.ValidationError({
                    'sub_total': 'El sub total es obligatorio y debe ser mayor o igual a 0 para ventas simplificadas.'
                })
            
            # Verificar si debe ser IVA = 0 (movcli)
            tipo_documento = cleaned_data.get('tipo_documento')
            es_movcli = tipo_documento and tipo_documento.codigo == 'movcli'
            ...
            if es_movcli:
                ...
                if iva is None or iva < 0:
                    raise forms.ValidationError({
                        'iva': 'El IVA es obligatorio y debe ser mayor o igual a 0 para ventas simplificadas.'
                    })
    ...
    class Meta:
        model = VentasCabezal
        fields = [
            'id_cliente', 'tipo_documento', 'serie_documento', 'numero_documento',
            'forma_pago', 'fecha_documento', 'moneda',
            'precio_iva_inc', 'plazo', 'fecha_vencimiento', 'disponibilidad',
            'tipo_venta', 'sub_total', 'iva', 'importe_total',
            'observaciones'
        ]
        widgets = {
            ...
            'precio_iva_inc': forms.Select(attrs={'class': 'form-control form-control-sm', 'id': 'id_precio_iva_inc', 'disabled': True, 'style': 'background-color: #e9ecef; cursor: not-allowed;'}, choices=[('SI', 'Sí'), ('NO', 'No')]),
...
class VentasLineasForm(forms.ModelForm):
    """Formulario para cada línea de venta"""
...
class VentasLineasFormSetBase(inlineformset_factory(
    VentasCabezal,
    VentasLineas,
    form=VentasLineasForm,
    ...
    """Formset personalizado para líneas de venta"""
...
VentasLineasFormSet = VentasLineasFormSetBase
