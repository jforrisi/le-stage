...
class ComprasDevolucionesCabezalForm(forms.ModelForm):
    """Formulario para el cabezal de devolución de compra"""
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'tipo_documento' in self.fields:
            self.fields['tipo_documento'].queryset = Documento.objects.filter(
                codigo__in=['ncimpo', 'ncprov' ,'devmovprov']
            )
            # Si es nueva compra, establecer 'ncprov' por defecto
            if not self.instance.pk:
                try:
                    doc_default = Documento.objects.get(codigo='ncprov')
        ...
        # Establecer tipo_compra siempre a CONVENCIONAL y ocultarlo
        if 'tipo_compra' in self.fields:
            self.fields['tipo_compra'].initial = 'CONVENCIONAL'
            # Ocultar el campo
            self.fields['tipo_compra'].widget = forms.HiddenInput()
    ...
    def clean(self):
        ...
        # Forzar tipo_compra siempre a CONVENCIONAL
        cleaned_data['tipo_compra'] = 'CONVENCIONAL'
    ...
    class Meta:
        ...
        fields = [ 'id_proveedor', 'tipo_documento',
                    'serie_documento', 'numero_documento', 'forma_pago',
                    'fecha_documento', 'moneda',
                     'precio_iva_inc', 'tipo_compra', 'disponibilidad',
...
class ComprasDevolucionesLineasForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ['linea', 'id_articulo', 'id_compra_linea', 'serie_doc_afectado', 'numero_doc_afectado', 'cantidad', 'precio']
        widgets = {
            ...
            'id_compra_linea': forms.HiddenInput(attrs={'id': 'id_compra_linea', 'class': 'id-linea-compra-hidden',}),
        ...
        labels = {
            ...
            'id_compra_linea': 'ID de Línea de Compra',
            'cantidad': 'Cantidad',
            'precio': 'Precio',
        }
...
class ComprasDevolucionesLineasFormSetBase(inlineformset_factory(
    ComprasDevolucionesCabezal,
    ComprasDevolucionesLineas,
    form=ComprasDevolucionesLineasForm,
    ...
    """Formset personalizado para líneas de devolución de compra"""
...
ComprasDevolucionesLineasFormSet = ComprasDevolucionesLineasFormSetBase
